...
Puppet::Functions.create_function(:'ibvs::vsphere::defer::update_vm_extraconfig') do
  ...
  def func(vsphere_settings, vms, vm_templates, reserved_ips)
    ...
    vms.each { |v|
      ...
      dc = vim.serviceInstance.find_datacenter(vm['datacenter']) || fail("Specified datacenter (#{vm['datacenter']}) was not found")
      ...
      template=vm_templates[vm['template']]['firstboot_script']
      template.gsub! '<%= $hostname %>', vm['name']
      template.gsub! "<%= $vm['interface'] %>", vm['interface']
      template.gsub! '<%= $ipaddress %>', "#{reserved_ip}/#{vm['network'].split(/\//)[1]}"
      template.gsub! "<%= $vm['gateway'] %>", vm['gateway']
      template.gsub! "<%= join($vm['dns'], ' ') %>", vm['dns'].join(' ')
      template.gsub! "<%= $puppet['server'] %>", vsphere_settings['puppet']
      template.gsub! "<%= $vm['puppet_psk'] %>", vm['puppet_psk']
      template.gsub! "<%= $vm['puppet_role'] %>", vm['puppet_role']
